...
def fetch(url, source_id, fixtures):
    if fixtures:
    ...
    with urlopen(Request(url, headers={"User-Agent": UA, "Accept": "*/*"}), timeout=20) as r:
...
def accept_terror(found, contextual, last_known):
    """A page redesign must never be able to invent a Critical. Accept a reading only if it is a clear statement
    within one step of the last confirmed level, or a loose match that simply agrees with it."""
    if not last_known or last_known not in TERROR_ORDER:
        return contextual
    if not contextual:
        return found == last_known
    return abs(TERROR_ORDER.index(found) - TERROR_ORDER.index(last_known)) <= 1
...
def build_checks(official, baseline, now):
    ...
    if not official.get("checks"):
        return []
    return [official["checks"]["alerts"], check_terror(official, baseline, now), official["checks"]["grid"]]


def build_feed(baseline, signals, notice, official, now, history=None, stale_days=60):
    """official = {'terror': str|None, 'items': [...], 'weather': {...}, 'used': [...], 'issues': [...]}"""
    tmap = baseline["official_terror_map"]
    areas, levels = [], {}
    for a in AREAS:
        b = baseline["areas"][a]
        floor = tmap.get(official.get("terror"), 1) if a == "security" else 1
        bump, why, _url = official.get("bumps", {}).get(a, (1, "", ""))
        lvl = max(b["level"], floor, bump)
        reason = b["reason"] + (f" Official UK threat level: {official['terror']}." if a == "security" and official.get("terror") else "")
        if bump > max(b["level"], floor):
            reason += f" Raised automatically to {LEVEL_NAMES[bump]} for up to 72 hours: {why}."
        levels[a] = lvl
        areas.append({"id": a, "name": AREA_NAMES[a], "level": lvl, "baseline": b["level"], "live_level": bump, "status": b["status"],
                      "reason": reason, "baseline_as_of": b["as_of"]})
...
def update_state(state, active_keys, now):
...
def gather_official(src_doc, fixtures, now, last_terror=None, state=None, hold_hours=6):
    ...
    for s in src_doc["sources"]:
        try:
            ...
            if s["kind"] == "mi5_level":
                ...
                if accept_terror(found, contextual, last_terror):
                    out["terror"] = found
                else:
                    print(f"warning: MI5 page read as {found} but the last confirmed level is {last_terror}. Ignored. Check the page by hand.", file=sys.stderr)
                    out["issues"].append("MI5 threat level (reading looked wrong, so the last confirmed level is shown)")
                    continue
            else:
...
def main():
    ...
    if a.editorial_only:
        ...
        official = gather_official(load_json(os.path.join(ROOT, "official_sources.json")), a.fixtures, now,
                                   baseline["official_terror_baseline"]["level"], state, float(cfg.get("hold_hours", 6)))
        os.makedirs(os.path.dirname(a.state), exist_ok=True)
        with open(a.state, "w", encoding="utf-8") as f:
            json.dump(update_state(state, official.get("active_keys", set()), now), f, indent=1)
