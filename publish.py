...
def build_feed(baseline, signals, notice, official, now, history=None, stale_days=60, decay_days=90, signal_max_age_days=365, retired=(), evidence_min=2, evidence_days=30):
    ...
    for a in AREAS:
        ...
        if ev and ev["count"] >= evidence_min and ev_level > base_eff:
            reason += (f" Raised to Elevated by official evidence: {ev['count']} different official statements in the last {evidence_days} days name hostile "
                       f"or state-linked activity affecting this area (listed below). The level falls back by itself as they age out.")
        elif ev and ev["count"] >= 1 and not follows:
            reason += f" {ev['count']} recent official statement noted (listed below). {evidence_min} different ones within {evidence_days} days raise this area to Elevated."
        if bump > base_eff:
        ...
        areas.append({"id": a, "name": AREA_NAMES[a], "level": lvl, "baseline": b["level"], "live_level": bump, "status": status,
                      "reason": reason, "baseline_as_of": b["as_of"], "lowered": lowered, "basis": basis,
...
def update_state(state, active_keys, now, terror_last=None, links=None, evidence=None):
...
def main():
    ...
    if a.editorial_only:
        ...
        with open(a.state, "w", encoding="utf-8") as f:
            json.dump(update_state(state, official.get("active_keys", set()), now, official.get("terror"), links_state or None, ev_store), f, indent=1)
    ...
    feed = build_feed(baseline, signals, load_json(os.path.join(ROOT, "editorial", "notice.json")), official, now, history,
                      int(cfg.get("stale_days", 60)), int(cfg.get("decay_days", 90)), int(cfg.get("signal_max_age_days", 365)), retired,
                      int(cfg.get("evidence_min_items", 2)), int(cfg.get("evidence_window_days", 30)))
